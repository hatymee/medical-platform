...
from app.models.models import Doctor, Invoice, InvoiceStatus, Payment, User
...
def record_payment(invoice_id: str, payload: PaymentCreate, db: Session = Depends(get_db), current_user: User = Depends(require_role("doctor", "clinic_admin"))):
    ...
    if Decimal(str(paid or 0)) + Decimal(str(payload.amount)) > Decimal(str(invoice.amount_due)):
        raise HTTPException(status_code=400, detail="Le paiement dépasse le montant restant")
...
def revenue_summary(start: date | None = None, end: date | None = None, db: Session = Depends(get_db), current_user: User = Depends(require_role("doctor", "clinic_admin"))):
    ...
    if doctor: invoices = invoices.filter(Invoice.doctor_id == doctor.id)
