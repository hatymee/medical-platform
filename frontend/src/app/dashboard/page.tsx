    ...
    <div className="dashboard">
      <aside className="dashboardSidebar">
        ...
        <nav className="dashboardMenu">
          <a className="active" href="/dashboard">Tableau de bord</a>
      ...
      <main className="dashboardMain">
        <header className="dashboardHeader">
          <div>
            ...
            <h1>Bonjour, {data.patient.first_name}.</h1>
            <p>Voici un résumé de votre suivi médical.</p>
          </div>
          <div className="patientAvatar">{data.patient.first_name[0]}{data.patient.last_name[0]}</div>
        </header>
        ...
        <section className="stats">
          ...
          <article>
            ...
            <small>issues de vos consultations</small>
          </article>
          <article>
            <span>Documents médicaux</span>
            <strong>{data.consultations.length}</strong>
            <small>consultations dans votre dossier</small>
          </article>
        </section>
        ...
        <section className="dashboardGrid">
          <article className="dashboardCard">
            <div className="cardTitle">
              ...
              <a href="/rendez-vous">Voir tout</a>
            </div>
            {upcoming.length ? upcoming.slice(0, 3).map((item) => (
              <div className="appointment" key={item.id}>
                <div className="dateBox">
                  ...
                  <span>{new Intl.DateTimeFormat("fr-FR", { month: "short" }).format(new Date(item.scheduled_at))}</span>
                </div>
                <div>
                  <h3>Consultation</h3>
                  <p>{displayDate(item.scheduled_at)}{item.reason ? ` · ${item.reason}` : ""}</p>
                </div>
              </div>
            )) : <p className="emptyText">Aucun rendez-vous à venir.</p>}
          </article>

          <article className="dashboardCard">
            <div className="cardTitle">
              <h2>Ordonnances récentes</h2>
              <a href="/dossier">Voir le dossier</a>
            </div>
            {data.prescriptions.length ? data.prescriptions.slice(0, 3).map((item) => (
              <div className="prescription" key={item.id}>
